...
function processResponse(data, url) {
  ...
  const res2 = res
    ...
    .replace(/href="(?!https?:\/\/)(\/?[^"]+)"/g, (match, path) => {
  ...
  return res2
    ...
    .replace(/url\("(?!https?:\/\/|data:image)(\/?[^"]+)"/g, (match, path) => {
...
app.get("/proxy", async (req, res) => {
  ...
  try {
    ...
    const markerScript = `
    <link rel="stylesheet" href="http://localhost:4002/inject.css">
    <script src="http://localhost:4002/inject.js"></script>
    `;
    html = html.replace(/<head>/, `<head>${markerScript}`);

    // Process the HTML content for links and resources
    const processedHTML = processResponse(html, "http://localhost:3001/proxy?url=" + new URL(targetUrl).origin);
...
app.get("/pa11y", async (req, res) => {
  ...
  res.json(processPa11yResults(pa11yResults));
});

function processPa11yResults(pa11yResults) {
  const results = { notice: {}, noticeCount: 0, warning: {}, warningCount: 0, error: {}, errorCount: 0 };
  pa11yResults.issues.forEach((result) => {
    if (result.type === "notice") {
      results.noticeCount++;
      processPrinciples(result, results.notice);
    } else if (result.type === "warning") {
      results.warningCount++;
      processPrinciples(result, results.warning);
    } else if (result.type === "error") {
      results.errorCount++;
      processPrinciples(result, results.error);
    }
  });
  return results;
}

processPrinciples = (item, collection) => {
  const principle = principles[item.code.split(".")[1]];
  collection[principle] ? collection[principle] : (collection[principle] = []);

  processGuideLines(item, collection[principle]);
};

processGuideLines = (item, collection) => {
  const guideline = wcagGuidelines[item.code.split(".")[2]];
  let obj = collection.filter((g) => g.guideline === guideline)[0];
  if (!obj) {
    obj = {
      guideline,
      totalElements: 0,
    ...
    collection.push(obj);
    obj.id = collection.length + ":" + guideline;
  }
  obj.totalElements++;
  item.id = (guideline+"-"+obj.totalElements).replace(/\s+/g, "");
...
function explainWCAGCode(code) {
  ...
  if (parts[3]) {
    ...
    const successCriterionName = wcagSuccessCriteria[formattedCriterion] || "Unknown Success Criterion";
    explanations[formattedCriterion] = successCriterionName;
  }

  if (parts[4]) {
    const techniqueCode = parts[4].split(".")[0];
    const techniqueDescription = wcagTechniques[techniqueCode] || "Unknown Technique";
  ...
  const techniqueDescriptions = techniqueList.map((t) => {
    ...
    return (
      wcagTechniques[t] ||
      wcagTechniques[`G${techniqueCode}`] ||
      wcagTechniques[`H${techniqueCode}`] ||
      "Unknown Technique"
    );
  });
